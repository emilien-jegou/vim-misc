...
function! vmisc#misc#os#is_mac() " {{{1
  ...
  if !exists('s:is_mac')
    ...
    if has('mac') || has('macunix') || has('gui_mac')
      ...
      let s:is_mac = 1
    elseif !vmisc#misc#os#is_win()
      " Otherwise we check the output of `uname' to avoid false negatives.
      let result = vmisc#misc#os#exec({'command': 'uname', 'check': 0})
...
function! vmisc#misc#os#is_win() " {{{1
  " Returns 1 (true) when on Microsoft Windows, 0 (false) otherwise.
  return has('win16') || has('win32') || has('win64')
endfunction

function! vmisc#misc#os#find_vim(...) " {{{1
  ...
  if exists('g:vmisc#misc#os#vim_progname')
    let pathname = g:vmisc#misc#os#vim_progname
  else
    let pathname = ''
  endif
  if empty(pathname) && vmisc#misc#os#is_mac()
    ...
    call vmisc#misc#msg#debug("vim-misc %s: Trying MacVim workaround to find Vim executable ..", g:vmisc#misc#version)
    let segments = vmisc#misc#path#split($VIMRUNTIME)
    if segments[-3:] == ['Resources', 'vim', 'runtime']
      let pathname = vmisc#misc#path#join(segments[0:-4] + ['MacOS', 'Vim'])
      call vmisc#misc#msg#debug("vim-misc %s: The MacVim workaround resulted in the Vim executable %s.", g:vmisc#misc#version, string(pathname))
    endif
  endif
  if empty(pathname)
    " Default logic.
    call vmisc#misc#msg#debug("vim-misc %s: Looking for Vim executable named %s on search path ..", g:vmisc#misc#version, string(program_name))
    let candidates = vmisc#misc#path#which(program_name)
    if !empty(candidates)
      call vmisc#misc#msg#debug("vim-misc %s: Found %i candidate(s) on search path: %s.", g:vmisc#misc#version, len(candidates), string(candidates))
      let pathname = candidates[0]
    endif
  endif
  call vmisc#misc#msg#debug("vim-misc %s: Reporting Vim executable %s.", g:vmisc#misc#version, string(pathname))
  return pathname
endfunction

function! vmisc#misc#os#exec(options) " {{{1
  ...
  try
    ...
    let is_win = vmisc#misc#os#is_win()
    ...
    if get(a:options, 'use_dll', 1) == 0
      ...
      let use_dll = vmisc#misc#os#can_use_dll()
    endif
    ...
    if has_key(a:options, 'stdin') && use_dll
      ...
      let cmd .= ' < ' . vmisc#misc#escape#shell(tempin)
    endif
    ...
    if redirect_output
      ...
      let cmd = printf('(%s) 1>%s 2>%s', cmd, vmisc#misc#escape#shell(tempout), vmisc#misc#escape#shell(temperr))
    endif

    " Use vim-shell or system() to execute the external command?
    if use_dll
      call vmisc#misc#msg#debug("vim-misc %s: Executing external command using compiled DLL: %s", g:vmisc#misc#version, cmd)
      let exit_code = vmisc#shell#execute_with_dll(cmd, async)
    else
      ...
      if async
        if is_win
          ...
          " implementation detail of vmisc#misc#os#exec() so I don't want to
          ...
          "
          " [1] https://github.com/vmisc/vim-misc/issues/17
        ...
        elseif has('unix')
          ...
          call vmisc#misc#msg#warn("vim-misc %s: I don't know how to execute the command %s asynchronously on your platform! Falling back to synchronous mode...", g:vmisc#misc#version, cmd)
        endif
      endif
      ...
      if has('unix')
        call vmisc#misc#msg#debug("vim-misc %s: Generated shell expression: %s", g:vmisc#misc#version, cmd)
        let cmd = printf('sh -c %s', vmisc#misc#escape#shell(cmd))
      endif

      " Let the user know what's happening (in case they're interested).
      if async && is_win
        call vmisc#misc#msg#debug("vim-misc %s: Executing external command using !start command: %s", g:vmisc#misc#version, cmd)
        silent execute '!' . cmd
      else
        call vmisc#misc#msg#debug("vim-misc %s: Executing external command using system() function: %s", g:vmisc#misc#version, cmd)
        let arguments = [cmd]
    ...
    if !async
      ...
      if get(a:options, 'check', 1) && exit_code != 0
        ...
        let msg = printf("vim-misc %s: External command failed with exit code %d!", g:vmisc#misc#version, result['exit_code'])
...
function! vmisc#misc#os#can_use_dll() " {{{1
  ...
  let can_use_dll = 0
  try
    let can_use_dll = vmisc#shell#can_use_dll()
  catch /^Vim\%((\a\+)\)\=:E117/
    " Silence E117.
  endtry
  return can_use_dll
...
function! s:readfile(fname, label, cmd) " {{{1
  try
    ...
    call vmisc#misc#msg#warn("vim-misc %s: Failed to read temporary file (%s) with %s of external command: %s! (external command: %s)", g:vmisc#misc#version, a:fname, a:label, v:exception, a:cmd)
    return []
  endtry
endfunction
