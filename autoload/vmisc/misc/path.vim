...
let s:windows_compatible = vmisc#misc#os#is_win()

function! vmisc#misc#path#which(...) " {{{1
  ...
  for program in a:000
    for directory in split($PATH, s:windows_compatible ? ';' : ':')
      let directory = vmisc#misc#path#absolute(directory)
      if isdirectory(directory)
        let found = 0
        for extension in extensions
          let path = vmisc#misc#path#merge(directory, program . extension)
          if executable(path)
        ...
        if s:windows_compatible && ! found
          ...
          " $PATHEXT.
          let path = vmisc#misc#path#merge(directory, program)
          if executable(path)
            call add(matches, path)
          endif
        endif
  ...
  return vmisc#misc#list#unique(matches)
endfunction

function! vmisc#misc#path#split(path) " {{{1
...
function! vmisc#misc#path#join(parts) " {{{1
  ...
  if type(a:parts) == type([])
    if s:windows_compatible
      return join(a:parts, vmisc#misc#path#directory_separator())
...
function! vmisc#misc#path#directory_separator() " {{{1
  " Find the preferred directory separator for the platform and settings.
  return exists('+shellslash') && &shellslash ? '/' : '\'
endfunction

function! vmisc#misc#path#absolute(path) " {{{1
  ...
  if type(a:path) == type('')
    ...
    elseif vmisc#misc#path#is_relative(path)
    ...
    let parts = vmisc#misc#path#split(path)
    if s:windows_compatible && parts[0] =~ '^[\/][\/]'
      ...
      let parts[0] = repeat(vmisc#misc#path#directory_separator(), 2) . parts[0][2:]
    ...
    return vmisc#misc#path#join(parts)
  endif
  return ''
endfunction

function! vmisc#misc#path#relative(path, base) " {{{1
  " Make an absolute pathname (the first argument) relative to a directory
  " (the second argument).
  let path = vmisc#misc#path#split(a:path)
  let base = vmisc#misc#path#split(a:base)
  while path != [] && base != [] && path[0] == base[0]
    call remove(path, 0)
    call remove(base, 0)
  endwhile
  let distance = repeat(['..'], len(base))
  return vmisc#misc#path#join(distance + path)
endfunction

function! vmisc#misc#path#merge(parent, child, ...) " {{{1
  " Join a directory pathname and filename into a single pathname.
  if type(a:parent) == type('') && type(a:child) == type('')
    " TODO Use vmisc#misc#path#is_relative()?
    if s:windows_compatible
...
function! vmisc#misc#path#commonprefix(paths) " {{{1
  " Find the common prefix of path components in a list of pathnames.
  let common = vmisc#misc#path#split(a:paths[0])
  for path in a:paths
    let index = 0
    for segment in vmisc#misc#path#split(path)
      if len(common) <= index
        break
  ...
  return vmisc#misc#path#join(common)
endfunction

function! vmisc#misc#path#starts_with(a, b) " {{{1
  ...
  let a = vmisc#misc#path#split(vmisc#misc#path#absolute(a:a))
  let b = vmisc#misc#path#split(vmisc#misc#path#absolute(a:b))
  return a[0 : len(b) - 1] == b
endfunction

function! vmisc#misc#path#encode(path) " {{{1
...
function! vmisc#misc#path#decode(encoded_path) " {{{1
...
" vmisc#misc#path#equals(a, b) - Check whether two pathnames point to the same file. {{{1

if s:windows_compatible
  function! vmisc#misc#path#equals(a, b)
    return a:a ==? a:b || vmisc#misc#path#absolute(a:a) ==? vmisc#misc#path#absolute(a:b)
  endfunction
else
  function! vmisc#misc#path#equals(a, b)
    return a:a ==# a:b || vmisc#misc#path#absolute(a:a) ==# vmisc#misc#path#absolute(a:b)
...
function! vmisc#misc#path#is_relative(path) " {{{1
...
function! vmisc#misc#path#tempdir() " {{{1
  ...
  if !exists('template')
    throw "vmisc#misc#path#tempdir() hasn't been implemented on your platform!"
  endif
  while 1
